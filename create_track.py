...
def add_track_to_tracks(track, tracks, id, random_range, bb_skip_percent):
    for i, p in enumerate(track):
        ...
        if i < len(tracks):
            tracks[i]["data"].append(
                {"cb_id": id, "bounding_box": bounding_box, "x": p["x"], "y": p["y"], "track_id": None}
            )
        else:
            tracks.append(
                {
                    "frame_id": len(tracks) + 1,
                    "data": [{"cb_id": id, "bounding_box": bounding_box, "x": p["x"], "y": p["y"], "track_id": None}],
                }
            )
    return tracks
...
def create_track(
    plot: bool = False, seed: int = 42, tracks_amount: int = 10, random_range: int = 10, bb_skip_percent: float = 0.25
):
    """
    # количество объектов
    tracks_amount = 10
    ...
    while i < tracks_amount:
        ...
        track = [
            {"x": int(x), "y": int(y)}
            for x, y in zip(np.linspace(x[0], x[-1], num=num), fun(np.linspace(x[0], x[-1], num=num), *coef))
        ]
        ...
        if check_track(track):
            ...
            tracks = add_track_to_tracks(track, tracks, i, random_range, bb_skip_percent)
            i += 1

    if plot:
        plt.show()

    return tracks_amount, tracks


if __name__ == "__main__":
    create_track(plot=True)
